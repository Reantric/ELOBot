    ...
    async runCommand(interaction: ChatInputCommandInteraction, _Bot: Client): Promise<void> {
        ...
        try {
            ...
            if (outcome.kind === 'filled') {
                const embed = buildTradeEmbed(interaction, outcome.execution, 'BUY');
                await interaction.editReply({ embeds: [embed] });
            } else {
                const order = outcome.order;
                await interaction.editReply({
                    content: `🕒 Placed BUY limit order for ${order.symbol} ${order.expiration} ${order.right} ${order.strike} @ ≤ ${order.limitPrice.toFixed(2)}.` +
